...
def main(cfg: DictConfig):
        ...
        if is_icl:
            if cfg.use_extracted_cot_vector:
                ...
                if cfg.use_base_vector:
                    ...
                    folder_name = "500mathvista_base_vector" if cfg.use_extracted_cot_vector_type == "licv" else "mimic_base_vector"
                elif cfg.use_extracted_cot_vector_type == "licv":
                # 新增 licv 版本处理：使用固定的 'licv_version' 作为文件夹名
                    folder_name = "500mathvista_licv_vector"
                elif cfg.use_extracted_cot_vector_type == "mimic":
                    folder_name = "500mathvista_mimic_vector"
                else:
    ...
    elif "qwen2.5" in cfg.model_name.lower() or "qwen" in cfg.model_name.lower():
    ...
    if cfg.use_extracted_cot_vector:
        ...
        if cfg.use_base_vector and loaded_vectors["encoder_type"]._target_ == "src.shift_encoder.AttnApproximator":
            print(
        ...
        vector_type = getattr(cfg, "use_extracted_cot_vector_type", "")
        if (
            (peft_name == "licv" or vector_type == "licv")
            and loaded_vectors["encoder_type"]._target_ == "src.shift_encoder.AttnApproximator"
        ):
        ...
        with torch.no_grad():  # Ensure these assignments don't get recorded for gradients
            if loaded_vectors["encoder_type"]._target_ == "src.shift_encoder.AttnFFNShift":
                print("Using AttnFFNShift for extracted vector evaluation.")
                print("ffn_cot_vector 的 shape:", loaded_vectors["ffn_cot_vector"].shape)
                print("ffn_hs_vector 的 shape:", loaded_vectors["ffn_hs_vector"].shape)
                if cfg.use_base_vector:
