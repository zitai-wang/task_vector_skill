...
from transformers import AutoModel, AutoModelForCausalLM, AutoTokenizer, AutoProcessor, Qwen2_5_VLForConditionalGeneration, LlavaOnevisionForConditionalGeneration
...
@hydra.main(config_path="config", config_name="eval_base.yaml", version_base=None)
def main(cfg: DictConfig):
    # initialize dataset and variables
    runname = get_expand_runname(cfg)

    if getattr(cfg, "ckpt_path", None) and getattr(cfg, "use_extracted_cot_vector", False):
        print(
            "[eval_base] ckpt_path is set; overriding use_extracted_cot_vector=True "
            "so learned-ckpt injection takes precedence."
        )
        cfg.use_extracted_cot_vector = False
    ...
    if mode == "generate_self_cot":
        ...
        if is_icl:
            ...
            epoch = int(epoch[0])
            record_name = getattr(cfg, "record_name", None)
            record_filename = record_name if record_name else f"epoch-{epoch}.json"
            record_path = os.path.join(record_dir, record_filename)
    ...
    # ... 原有的 if "llama-3.1-8b-instruct" in cfg.model_name 逻辑 ...
        ...
        internvl_model_args = {
            ...
            "use_flash_attn": bool(getattr(cfg, "use_flash_attn", True)),
        }
        use_device_map_auto = bool(getattr(cfg, "device_map_auto", False))
        if use_device_map_auto:
            internvl_model_args["device_map"] = "auto"
        model = InternVLModelWrapper(
            ...
            model_args=internvl_model_args,
            input_size=int(getattr(cfg, "image_size", 448)),
            max_num_tiles=int(getattr(cfg, "max_num_tiles", 12)),
            model_name=cfg.model_name,
        )
    ...
    if cfg.use_extracted_cot_vector:
        ...
        encoder_target = getattr(loaded_vectors.get("encoder_type"), "_target_", "")
        if encoder_target not in {
            "src.shift_encoder.AttnApproximator",
            "src.shift_encoder.AttnFFNShift",
        }:
            if cfg.use_base_vector and loaded_vectors.get("ffn_hs_vector") is not None:
                print(
                    f"use_base_vector=True with non-shift encoder target {encoder_target!r}. "
                    "Switching encoder target to AttnFFNShift so evaluation uses ffn_hs_vector."
                )
                loaded_vectors["encoder_type"]._target_ = "src.shift_encoder.AttnFFNShift"
            elif loaded_vectors.get("attn_cot_vector") is not None:
                print(
                    f"Detected non-shift encoder target {encoder_target!r}. "
                    "Switching encoder target to AttnApproximator so evaluation uses attn_cot_vector."
                )
                loaded_vectors["encoder_type"]._target_ = "src.shift_encoder.AttnApproximator"
            encoder_target = getattr(loaded_vectors.get("encoder_type"), "_target_", "")
        ...
        if cfg.use_base_vector and encoder_target == "src.shift_encoder.AttnApproximator":
            print(
                "use_base_vector=True detected with AttnApproximator checkpoint. "
                "Switching encoder target to AttnFFNShift so evaluation uses ffn_hs_vector."
            )
            loaded_vectors["encoder_type"]._target_ = "src.shift_encoder.AttnFFNShift"
            encoder_target = "src.shift_encoder.AttnFFNShift"

        peft_name = getattr(getattr(cfg, "peft", None), "name", "")
        if peft_name == "licv" and encoder_target == "src.shift_encoder.AttnApproximator":
            print("The shift encoder version conflict! Now we change it to LICV version.")
            loaded_vectors["encoder_type"]._target_ = "src.shift_encoder.AttnFFNShift"
            encoder_target = "src.shift_encoder.AttnFFNShift"

        if encoder_target == "src.shift_encoder.AttnApproximator":
            if loaded_vectors["attn_cot_vector"] is not None:
                ...
                if loaded_vectors["multi_head_attn_strategy"] and encoder_target == "src.shift_encoder.AttnApproximator":
            ...
            if getattr(cfg, "use_static_mu", False) and encoder_target == "src.shift_encoder.AttnApproximator":
        ...
        if (
            loaded_vectors.get("ffn_hs_vector") is not None
            or loaded_vectors.get("ffn_cot_vector") is not None
        ) and encoder_target == "src.shift_encoder.AttnFFNShift":
        ...
        with torch.no_grad():  # Ensure these assignments don't get recorded for gradients
            if encoder_target == "src.shift_encoder.AttnFFNShift":
                if cfg.use_base_vector:
    ...
    try:
        ...
        if mode == "generate_self_cot":
            ...
            with open(full_output_path, 'w', encoding='utf-8') as f:
                for record in result: # 'result' here is the list of records from dataset.eval
                    ...
                    ground_truth = record.get("extracted_ground_truth", record.get("ground_truth"))
                    self_cot_record = {
                        "question": record["question"],
                        "self_cot": record["model_output"], # Model output is the generated CoT
                        "is_correct": record["is_correct"],
                        "gt_answer": ground_truth,
                        "gt_numerical": ground_truth,
                    }
                    if "options" in record:
                        self_cot_record["options"] = record["options"]
        ...
        if record_path and os.path.exists(record_path) and os.path.getsize(record_path) == 0:
