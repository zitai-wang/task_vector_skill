# -*- coding: utf-8 -*-
# 这个文件专门用来测试baseline在 max_new_tokens=1024 的设置下的表现，主要是为了验证是否存在之前怀疑的 1024 长度限制问题。
...
@hydra.main(config_path="config", config_name="eval_ffn.yaml", version_base=None)
def main(cfg: DictConfig):
    ...
    if mode == "generate_self_cot":
        ...
        if is_icl:
            if cfg.use_extracted_cot_vector:
                ...
                max_tokens = getattr(cfg.generation_args, "max_new_tokens", None)
                # Construct base filename parts
                file_name_parts = [
                    f"{cfg.data.num_shot}shot",
                ]
                if max_tokens == 1024:
                    file_name_parts.append("5_baseline")
    ...
    if cfg.use_extracted_cot_vector:
        ...
        loaded_vectors = torch.load(
            cfg.extracted_cot_vector_path,
            map_location=device,
            weights_only=False,
        )
