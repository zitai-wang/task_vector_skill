...
#     print(file_path.split("record/")[-1])
...
#     parser.error("--result_file is required for 'accuracy' mode.")

# postprocess_single(args.result_file)

# # result_file_format = args.result_file
# # result_file_post = "-dft_False-epoch_1-llama-3.1-8b-instruct-gsm8k-2000/epoch-0.json"
# # for i in range(0, 32):
# #     result_file = result_file_format + str(i) + result_file_post
# #     postprocess_single(result_file)

# result_file_format = "/data1/wzy/cot-mimic/results/record/math_licv_version/licv_layers_"
# result_file_post = ".json" # 根据你提供的文件名，中间只有数字

# for i in range(0, 32):
#     # 拼接完整的文件名
#     result_file = result_file_format + str(i) + result_file_post
#     print(f"Processing file: {result_file}") # 打印当前处理的文件名，方便调试
#     postprocess_single(result_file)
...
# if not args.result_file:
#     # 动态处理 0-31 层
#     base_dir = "/data1/wzy/cot-mimic/results/record/math_licv_version/"
#     for i in range(0, 32):
#         file_name = f"licv_layers_{i}_direct_q_1.0.json"
...
if not args.result_file:
    # 动态处理 0-31 层
    base_dir = "/data1/wzy/cot-mimic/results/record/hard_math_licv_version/"
    for i in range(0, 32):
        file_name = f"licv_layers_{i}_direct_q_1.0.json"
        full_path = f"{base_dir}{file_name}"
        postprocess_single(full_path)
