...
RUN_TAG=qwen2.5-7b-instruct_commonsenseqa_mimic_20260527_154105
RUN_DIR=$ROOT/results/static_pipeline/$RUN_TAG
LOG_DIR=$RUN_DIR/logs
BASELINE_RECORD_DIR=$RUN_DIR/baseline_records
RECORD_DIR=$RUN_DIR/records
VECTOR=$RUN_DIR/$RUN_TAG.pt

mkdir -p "$HF_HOME/datasets" "$HF_HOME/transformers" "$RUN_DIR" "$LOG_DIR" "$BASELINE_RECORD_DIR" "$RECORD_DIR"

stdbuf -oL -eL /home/wzy/anaconda3/envs/licv/bin/python \
  $ROOT/src/eval_baseline.py \
  --config-name eval_baseline \
  ...
  +record_root=$BASELINE_RECORD_DIR \
  > $LOG_DIR/00_baseline.log 2>&1
