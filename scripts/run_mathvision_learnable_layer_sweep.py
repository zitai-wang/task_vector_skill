...
LOG_ROOT = ROOT / "scripts" / "mathvision_learnable_layer_sweep"
...
RUN_PREFIX = "mathvision-qwen2.5-vl-mimic"
...
def expand_runname(layer: int) -> str:
    return f"{runname_for_layer(layer)}-qwen2.5-vl-7b-instruct-mathvision-500"
...
def build_eval_cmd(python_bin: str, layer: int, ckpt_path: Path) -> list[str]:
    return [
        ...
        "data.name=mathvision",
        "data.source=MathLLMs/MathVision",
        "data.query_split=testmini",
        "data.support_split=testmini",
        ...
        "generation_args.max_new_tokens=4096",
...
def print_summary(layers: list[int]) -> None:
    ...
    print("\nCurrent MathVision learnable layer sweep summary:")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train and evaluate learnable MathVision CoT vectors layer by layer.")
    ...
    print(f"Launching MathVision learnable sweep for layers: {layers}")
