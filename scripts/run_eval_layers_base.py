...
@hydra.main(config_path="../src/config", config_name="eval_base.yaml", version_base=None)
def main(cfg: DictConfig):
    ...
    for current_layers in layers_to_iterate:
        ...
        # 直接透传当前配置，避免数据集 split / 样本数 / base-vector 开关被默认值覆盖。
        hydra_overrides = [
            ...
            f"use_extracted_cot_vector_type={cfg.use_extracted_cot_vector_type}",
            ...
            f"use_base_vector={cfg.use_base_vector}",
            f"+encoder={cfg.use_extracted_cot_vector_type}",
            f"+peft={cfg.use_extracted_cot_vector_type}",
            f"only_shift_at_layer={current_layers}",
        ]
        if getattr(cfg.data, "source", None):
            hydra_overrides.append(f"++data.source={cfg.data.source}")
        if getattr(cfg.data, "query_split", None):
            hydra_overrides.append(f"++data.query_split={cfg.data.query_split}")
        if getattr(cfg.data, "support_split", None):
            hydra_overrides.append(f"++data.support_split={cfg.data.support_split}")
        if getattr(cfg, "record_dir_tag", None):
            hydra_overrides.append(f"+record_dir_tag={cfg.record_dir_tag}")
        if getattr(cfg, "record_root", None):
            hydra_overrides.append(f"++record_root={cfg.record_root}")
        
        # 构建完整的命令行，直接调用 eval.py
        command = [
            sys.executable,
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "src", "eval_base.py"),
        ] + hydra_overrides
        if cfg.data.name == "scienceqa":
            command[2:2] = ["--config-name", "eval_scienceqa"]

        run_env = os.environ.copy()
        if cfg.devices:
            command.append(f"devices={cfg.devices}")
            run_env["CUDA_VISIBLE_DEVICES"] = str(cfg.devices)
            print(
                f"Binding subprocess to CUDA_VISIBLE_DEVICES={cfg.devices} "
                f"(eval_base will use logical cuda:0 on this visible GPU set)."
            )
        else:
            print("No explicit devices provided; subprocess will use current environment GPU visibility.")

        print(f"Running command: {' '.join(command)}")

        project_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..")
        process = subprocess.Popen(command, cwd=project_root, text=True, env=run_env)
        process.wait()
