...
def main(cfg: DictConfig):
    ...
    for current_layers in layers_to_iterate:
        ...
        # run_name_parts = [
        #     f"layer_eval_post_train",
        #     f"{use_extracted_cot_vector_type}",
        #     f"{dataset}",
        #     f"{model_name}",
        #     f"{num_shots}shot",
        ...
        record_dir = os.path.join(result_dir, "record", extracted_name)
        ...
        if cfg.trained_with_self_cot:
            ckpt_path_name_parts.append("sc")

        ckpt_path_name_parts += [f"{model_name}", f"{dataset}", "3000"]

        runname = "-".join(ckpt_path_name_parts)

        if cfg.only_alignment_loss:
            runname += "-only_alignment_loss"
